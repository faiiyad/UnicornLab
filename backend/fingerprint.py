"""
fingerprint.py
Compares the crime-scene fingerprint against every suspect fingerprint
using ORB feature matching with significant noise-robustness improvements.
Saves results to root/data/fingerprint_results.json

Improvements over the original starter:
  1. Image preprocessing pipeline:
       a. Grayscale conversion (was already done)
       b. CLAHE (Contrast Limited Adaptive Histogram Equalisation) —
          dramatically improves ridge visibility on low-contrast or
          partially smudged prints without blowing out highlights.
       c. Gaussian blur — suppresses high-frequency noise before
          feature detection so ORB doesn't latch onto noise spikes.
       d. Adaptive thresholding — binarises the ridge pattern
          consistently regardless of uneven ink/lighting across the print.
  2. Lowe's ratio test (knnMatch, k=2) replaces the raw distance
     threshold.  A match is "good" only when the best match is
     significantly closer than the second-best (ratio < 0.75).
     This eliminates most false positives caused by noise.
  3. Geometric consistency check (homography via RANSAC) — verifies
     that the good matches form a geometrically consistent transformation.
     Inlier count is used as the final score, filtering out any remaining
     coincidental feature matches.
  4. Normalisation relative to the top scorer → [0, 1].
"""
...
def preprocess(img: np.ndarray) -> np.ndarray:
    ...
    # 2. CLAHE — clip limit 2.0, tile 8x8 are standard for fingerprints
    clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
    enhanced = clahe.apply(gray)

    # 3. Gaussian blur — 3x3 kernel keeps ridges sharp while killing noise
    blurred = cv2.GaussianBlur(enhanced, (3, 3), 0)

    # 4. Adaptive threshold — handles uneven illumination across the print
...
# ─────────────────────────────────────────────
# Matching with Lowe ratio test + RANSAC
# ─────────────────────────────────────────────
def match_prints(crime_kp, crime_des, suspect_kp, suspect_des) -> dict:
    ...
    """
    # knnMatch with k=2 to enable Lowe ratio test
    ...
    # Homography (RANSAC) — geometric consistency check
...
def analyse_fingerprints() -> dict:
    ...
    # Normalise using inlier_count (more reliable than raw good_matches)
